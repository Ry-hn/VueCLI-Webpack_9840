        ...
        <v-card>
            ...
            <v-data-table :headers="headers" :items="todos" :search="search">
                ...
                <template  v-slot:[`item.actions`]="{ item }">
                    ...
                    <input type="checkbox" id="checkbox" v-model="item.hapus" @change="checked(item)">
                </template>
            </v-data-table>
        </v-card>

    <v-card v-if="todos3.length > 0">
        <v-card-title>
            Delete Multiple: 
        </v-card-title>
        <v-card-text>
            <ul v-for="(todoz, index) in todos3" :key="index">
                <li>{{todoz.task}}</li>
            </ul>
        </v-card-text>

        <v-btn color="red" style="margin:20px" @click="delAll">HAPUS SEMUA</v-btn>
    </v-card>
...
export default {
    ...
    methods: {
        ...
        checked(item) {
            if(item.hapus) {
                this.todos3.push(item);
            }
            else {
                let index = this.todos3.findIndex(todo => todo.task === item.task);
                this.todos3.splice(index, 1);
            }

            console.log(this.todo3);
        },
        delAll() {
            this.hapusAll();

            this.todos = this.todos.filter( todo => !this.todos3.includes(todo));
            this.todos3.length = 0;
        },
        hapusAll() {
            this.todos3.forEach(todo => this.todos2.push(todo));
        }
    }
}
</script>